...
from student_app.models import Grade, Student, Subject
...
class Test_student(TestCase):
    ...
    def test_019_student_serializer_all_with_proper_reponse(self):
        ...
        # to get subjects data we need to have the id saved in database
        stud.save()
        self.assertEquals(
            serializer.data,
            {
    ...
    def test_022_subject_with_improper_subject_format(self):
        try:
            ...
            a_subject.save()
            a_subject.add_student(Student.objects.all().first().id)
